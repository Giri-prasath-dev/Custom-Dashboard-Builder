...
const AreaChartWidget = ({ title, config }) => {
  ...
  const { data, isLoading, error } = useQuery({
    queryKey: ['areaChartData', xField, yField, aggregation, dateRange],
  ...
  return (
    <div className="h-full w-full min-h-0 overflow-hidden">
      <ResponsiveContainer width="100%" height="100%">
        <AreaChart data={chartData} margin={{ top: 20, right: 10, left: 45, bottom: 30 }}>
          ...
          />
          <Area type="monotone" dataKey="value" stroke={chartColor} fill={chartColor + '40'}>
            {showLabels && (
